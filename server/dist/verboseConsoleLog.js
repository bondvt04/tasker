...
logger = {
    log: function log() {
        this.doAction({ loggerArguments: arguments, action: "log" });
    },
    error: function error() {
        this.doAction({ loggerArguments: arguments, action: "error" });
    },
    warn: function warn() {
        this.doAction({ loggerArguments: arguments, action: "warn" });
    },
    debug: function debug() {
        this.doAction({ loggerArguments: arguments, action: "debug" });
    },

    doAction: function doAction(options) {
        var self = this;
    ...
    _isValidFileName: function _isValidFileName(fileName) {
        if (!fileName || fileName && fileName.match(/v8-debug/) || fileName && fileName.match(/ConsoleAgent/) || fileName && fileName.match(/verboseConsoleLog/)) {
            return false;
        }
